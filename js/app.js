...
function refreshCampaignSchedulesInBackground(delayMs = 900) {
  ...
  dashboardScheduleRefreshTimer = setTimeout(async () => {
    ...
    try {
      ...
      campaignScheduleState.error = null;
      ...
      campaignScheduleState.error = error;
...
async function toggleUserSubscription(userId, nextState, button) {
  ...
  if (!await openDashboardConfirm({title:`${unsubscribed?"Suppress":"Resubscribe"} contact?`,message:`${action} ${user.emailAddress}?`,confirmLabel:unsubscribed?"Suppress":"Resubscribe",destructive:unsubscribed})) return;
...
let campaignContentLoadPromise=null;
let mainComposeRenderSequence=0;
...
function getCampaignLifecycleStatus(campaign) {
  const raw=getRawCampaignStatus(campaign);
  if(['PAUSED','COMPLETED','CANCELED','ARCHIVED'].includes(raw))return raw;
  if(!campaignScheduleState.loaded){
    // Saved status remains visible while schedule details load.
    if(raw==='DRAFT'||raw==='SCHEDULED'||raw==='RUNNING')return raw;
    return campaignScheduleState.error?'SCHEDULE_UNAVAILABLE':'CHECKING';
  }
...
function getCampaignManagementRows() {
  ...
  return data.campaigns
    ...
    .filter(campaign => {
      const hasUpcoming = !['CANCELED','COMPLETED','ARCHIVED'].includes(getRawCampaignStatus(campaign)) && (campaignScheduleState.schedules || []).some(schedule =>
        String(schedule.campaignId) === String(campaign.campaignId) &&
        String(schedule.scheduleStatus).toUpperCase() === 'UPCOMING'
      );
...
function updateCampaignLifecycleCounts(
  ...
  setText('campaignLifecycleScheduledCount',campaignScheduleState.loaded?count('SCHEDULED').toLocaleString():'…');
...
function renderCampaignNextSchedule_(campaignId){const campaign=getCampaignById(campaignId);if(['CANCELED','COMPLETED','ARCHIVED'].includes(getRawCampaignStatus(campaign)))return '<span class="muted-id">—</span>';if(!campaignScheduleState.loaded)return `<span class="campaign-schedule-pending">${campaignScheduleState.error?'Schedule unavailable':'Checking schedule…'}</span>`;const rows=(campaignScheduleState?.schedules||[]).filter(s=>String(s.campaignId)===String(campaignId)&&String(s.scheduleStatus).toUpperCase()==='UPCOMING').sort((a,b)=>`${a.startDate} ${a.startTime}`.localeCompare(`${b.startDate} ${b.startTime}`));if(!rows.length)return '<span class="muted-id">—</span>';const s=rows[0],more=rows.length-1;return `<div class="campaign-next-schedule"><strong>${escapeHtml(formatCampaignScheduleDate_(s.startDate))}</strong><span>${escapeHtml(s.startTime)}${more?` · +${more} more`:''}</span></div>`;}
...
function showCampaignsNotice(
  ...
  notice.textContent =
    message ||
    '';
  if(campaignsNoticeTimer)clearTimeout(campaignsNoticeTimer);
  if(message)campaignsNoticeTimer=setTimeout(()=>{notice.hidden=true;notice.textContent='';campaignsNoticeTimer=null;},6500);
}
...
async function updateCampaignLifecycle(
  ...
  if (destructive) {
    ...
    const confirmed =
      await openDashboardConfirm({title:"Cancel campaign?",message:`"${campaign.campaignName}" will no longer be treated as active.`,confirmLabel:"Cancel Campaign",destructive:true});
...
function renderCampaignBuilderCurrentAudience() {
  ...
  if (!members.length) {
    tbody.innerHTML = emptyRow(5, 'No recipients have been added yet.');
    return;
  }
  tbody.innerHTML = members.map(member => {
    const user = getCampaignMemberUser(member);
    return `<tr><td><strong>${escapeHtml(user?.firstName || '—')}</strong></td><td>${escapeHtml(user?.emailAddress || member.emailAddress || '—')}</td><td>${escapeHtml(user?.company || '—')}</td><td><span class="status-badge badge-success">Included</span></td><td><button type="button" class="secondary-action-button compact" data-builder-remove-member="${escapeHtml(member.campaignMemberId||'')}" title="Remove from this campaign only">Remove</button></td></tr>`;
  }).join('');
}
...
function attachCampaignBuilderListeners() {
  ...
  document.getElementById('campaignBuilderCurrentAudienceTable')?.addEventListener('click',async event=>{
    const button=event.target.closest('[data-builder-remove-member]'); if(!button)return;
    const id=button.dataset.builderRemoveMember;
    const member=getCampaignBuilderMembers().find(m=>String(m.campaignMemberId)===id);
    if(!member)return;
    const user=getCampaignMemberUser(member);
    const identity=user?.emailAddress||member.emailAddress||'this recipient';
    if(!await openDashboardConfirm({title:'Remove recipient from campaign?',message:identity+' will no longer receive unsent email from this campaign. The contact remains in Contacts and saved lists.',confirmLabel:'Remove from Campaign'}))return;
    await withActionButtonBusy(button,'Removing…',async()=>{
      try{
        await DashboardApi.setCampaignMemberStatus(id,'INACTIVE');
        campaignBuilderSelectedUserIds.delete(String(member.userId||''));
        await initDashboard(true);
        renderCampaignBuilderRecipients();
        showCampaignBuilderNotice('Recipient removed from this campaign. The contact remains in Contacts.','success');
      }catch(error){showCampaignBuilderNotice(error?.message||'Could not remove this recipient.','error');}
    });
  });
...
function populateMainComposeStatusFilter() {
  ...
  select.innerHTML = `
    ...
    <option value="ready">Written</option>
...
async function renderMainComposeWorkspace(force=false) {
  ...
  try {
    ...
    setText('composeMainSummary',`${campaigns.length} campaign${campaigns.length===1?'':'s'} · ${readyCount} email${readyCount===1?'':'s'} written · ${campaigns.length-readyCount} not written`);

    if (!campaigns.length) {
      body.innerHTML = '<tr><td colspan="6" class="compose-main-empty"><strong>No campaign emails found.</strong><span>Try another search or email-status filter.</span></td></tr>';
      return;
    }
    ...
    body.innerHTML = campaigns.map(campaign => {
      const content = getMainComposeContentForCampaign(campaign.campaignId);
      const hasContent = Boolean(content && content.subject && (content.plainBody || content.htmlBody));
      const closed=['CANCELED','COMPLETED','ARCHIVED'].includes(getRawCampaignStatus(campaign));
      ...
      return `<tr>
        ...
        <td>${campaignLifecycleBadge(campaign)}</td>
        ...
        <td class="compose-main-action-col">
          ...
          >${closed?'View Email':hasContent?'Edit Email':'Write Email'}</button>
        </td>
      </tr>`;
    }).join('');
  } catch (error) {
    if(renderId!==mainComposeRenderSequence)return;
    setText('composeMainSummary','Campaign emails unavailable');
...
async function ensureCampaignContentLoaded(force=false) {
  if(campaignContentLoadPromise)return campaignContentLoadPromise;
  if(campaignContentState.loaded&&!force)return campaignContentState;
  campaignContentLoadPromise=(async()=>{
    const [contentResult,templateResult]=await Promise.allSettled([
      DashboardApi.getCampaignContent(),DashboardApi.getEmailTemplates()
    ]);
    if(contentResult.status!=='fulfilled')throw contentResult.reason||new Error('Campaign emails could not load.');
    const response=contentResult.value||{};
    const result=response?.result?.result||response?.result||response?.data?.result||response?.data||{};
    if(!Array.isArray(result.campaignContent))throw new Error('Campaign email response is incomplete. Refresh to try again.');
    const campaignContent=result.campaignContent.map(normalizeCampaignContentRecord);
    let templates=campaignContentState.templates;
    if(templateResult.status==='fulfilled'){
      const templateResponse=templateResult.value||{};
      const templateData=templateResponse?.result?.result||templateResponse?.result||templateResponse?.data?.result||templateResponse?.data||{};
      if(Array.isArray(templateData))templates=templateData.map(normalizeEmailTemplateRecord);
      else if(Array.isArray(templateData.templates))templates=templateData.templates.map(normalizeEmailTemplateRecord);
    }else if(Array.isArray(result.templates))templates=result.templates.map(normalizeEmailTemplateRecord);
    campaignContentState={loaded:true,campaignContent,templates};
    return campaignContentState;
  })();
  try{return await campaignContentLoadPromise;}
  finally{campaignContentLoadPromise=null;}
}
...
async function sendCampaignComposeTest() {
  ...
  const testEmail = await openDashboardConfirm({title:'Send test email',message:'Enter the address that should receive the test.',inputLabel:'Email address',confirmLabel:'Send Test'});
...
async function saveComposeAsTemplate() {
  ...
  const name=await openDashboardConfirm({title:'Save as template',message:'Name this reusable email.',inputLabel:'Template name',confirmLabel:'Save Template'}); if(!name?.trim())return;
...
async function composeCampaignDelivery_(action){
  const campaign=getCampaignBuilderCampaign();
  if(!campaign)return;
  if(['PAUSED','COMPLETED','CANCELED','ARCHIVED'].includes(getRawCampaignStatus(campaign))){
    showCampaignComposeNotice('This campaign cannot send in its current status. Duplicate a closed campaign or resume a paused one.','error');return;
  }
  if(!await saveCampaignCompose())return;
  composeOnlyMode=false;
  document.getElementById('campaignBuilderWorkspace')?.classList.remove('compose-only');
  const label=document.querySelector('.campaign-builder-workspace-title .detail-eyebrow');
  if(label)label.textContent='Campaign Builder';
  const exit=document.getElementById('campaignBuilderExit');if(exit)exit.textContent='Exit Builder';
  switchCampaignBuilderStep('review',{skipReviewLoad:true});
  try{
    ...
    if(action==='schedule'){
      ...
      document.getElementById('campaignReviewScheduleDate')?.focus();
...
function openDashboardConfirm({
  ...
  const inputWrap=document.getElementById('dashboardConfirmInputWrap');
  const input=document.getElementById('dashboardConfirmInput');
  if(inputWrap)inputWrap.hidden=!inputLabel;
  setText('dashboardConfirmInputLabel',inputLabel);
  if(input)input.value=inputValue;
  document.getElementById('dashboardConfirmIcon')?.classList.toggle('is-destructive',destructive);
...
function closeDashboardConfirm(result=false) {
  const inputWrap=document.getElementById('dashboardConfirmInputWrap');
  const input=document.getElementById('dashboardConfirmInput');
  if(result&&inputWrap&&!inputWrap.hidden&&!input?.value.trim()){input?.focus();return;}
  const backdrop = document.getElementById('dashboardConfirmBackdrop');
  if (backdrop) {
  ...
  if(resolve)resolve(inputWrap&&!inputWrap.hidden?(result?input.value.trim():null):Boolean(result));
}
...
function attachDashboardConfirmListeners() {
  ...
  document.getElementById('dashboardConfirmInput')?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();closeDashboardConfirm(true);}});
...
async function duplicateTemplateLibraryItem(templateId) {
  ...
  let name =
    await openDashboardConfirm({title:'Duplicate template',message:'Choose a name for the copy.',inputLabel:'Template name',inputValue:`${template.name || 'Template'} Copy`,confirmLabel:'Duplicate'});
...
async function deleteCampaignFollowup(
  ...
  if (
    !await openDashboardConfirm({title:"Remove follow-up?",message:step.subject,confirmLabel:"Remove Follow-up",destructive:true})
  ) {
    return;
  }
...
async function runCampaignPrecheck() {
  ...
  if (
    !await openDashboardConfirm({title:"Run recipient pre-check?",message:`Check all ${activeMembers.length} active recipients in ${campaign.campaignName || campaign.campaignId}.`,confirmLabel:"Run Pre-check"})
  ) {
...
async function setCampaignMemberStatus(
  ...
  if (
    !await openDashboardConfirm({title:`${actionLabel} recipient?`,message:identity,confirmLabel:actionLabel,destructive:actionLabel.toLowerCase()==="deactivate"})
  ) {
...
async function deleteSelectedContactList() {
  ...
  const confirmed =
    await openDashboardConfirm({title:"Delete contact list?",message:`"${list?.name || 'this contact list'}" will be deleted. Contacts themselves will remain.`,confirmLabel:"Delete List",destructive:true});
...
async function deleteContactSegment(
  ...
  const confirmed =
    await openDashboardConfirm({title:"Delete segment?",message:`"${segment?.name || 'this segment'}" will be deleted.`,confirmLabel:"Delete Segment",destructive:true});
...
async function runPrecheckModuleCampaign() {
  ...
  if (
    !await openDashboardConfirm({title:"Run recipient pre-check?",message:`Check all ${activeCount} active recipients in ${campaign?.campaignName || campaignId}.`,confirmLabel:"Run Pre-check"})
  ) {
...
/* ============================================================
   ...
   ============================================================ */
let campaignScheduleState={loaded:false,loading:false,schedules:[],error:null}; let campaignScheduleListenersAttached=false;
...
function updateCampaignScheduleUi(){const terminal=['CANCELED','COMPLETED','ARCHIVED','PAUSED'].includes(getRawCampaignStatus(getCampaignBuilderCampaign()));const save=document.getElementById('campaignScheduleSave');if(save)save.disabled=terminal;const card=document.querySelector('.campaign-schedule-card');if(card)card.classList.toggle('campaign-delivery-locked',terminal);if(terminal)showCampaignScheduleNotice('This campaign is closed or paused. Duplicate a closed campaign or resume a paused campaign before scheduling.','warning');const s=getCurrentCampaignSettings()||normalizeCampaignSetting({}),tz=s.timezone;setText('campaignScheduleInlineTimezone',tz);setText('campaignScheduleSummaryTimezone',tz);setText('campaignScheduleSummaryHours',`${s.windowStart} – ${s.windowEnd}`);setText('campaignScheduleSummaryDays',s.sendingDays.join(', '));}
...
function renderCampaignScheduleList(){
 const list=document.getElementById('campaignScheduleList'),history=document.getElementById('campaignScheduleHistory');
 const rows=getCampaignSchedulesForCurrent_(),terminal=['CANCELED','COMPLETED','ARCHIVED'].includes(getRawCampaignStatus(getCampaignBuilderCampaign())),upcoming=rows.filter(x=>x.scheduleStatus==='UPCOMING'&&!terminal),past=rows.filter(x=>['COMPLETED','CANCELED'].includes(x.scheduleStatus));
 setText('campaignScheduleCount',String(upcoming.length));
 if(list)list.innerHTML=upcoming.length?upcoming.map(x=>`<div class="campaign-schedule-list-row"><div class="schedule-time-block"><strong>${escapeHtml(formatCampaignScheduleDate_(x.startDate))}</strong><span>${escapeHtml(x.startTime)}</span></div><div class="schedule-id-block"><small>${escapeHtml(x.campaignScheduleId)}</small>${scheduleStatusBadge_(x.scheduleStatus)}</div><div class="schedule-row-actions"><button type="button" class="secondary-action-button compact" data-schedule-edit="${escapeHtml(x.campaignScheduleId)}">Edit</button><button type="button" class="row-menu-trigger" data-schedule-menu-trigger="${escapeHtml(x.campaignScheduleId)}">•••</button><div class="row-menu-panel schedule-row-menu" data-schedule-menu="${escapeHtml(x.campaignScheduleId)}" hidden><button type="button" class="row-menu-item danger" data-schedule-cancel="${escapeHtml(x.campaignScheduleId)}">Cancel Schedule</button></div></div></div>`).join(''):'<div class="schedule-empty">No upcoming times. You can send now or schedule from Review.</div>';
 if(history)history.innerHTML=past.length?past.map(x=>`<div class="campaign-schedule-list-row"><div class="schedule-time-block"><strong>${escapeHtml(formatCampaignScheduleDate_(x.startDate))}</strong><span>${escapeHtml(x.startTime)}</span></div><div class="schedule-id-block"><small>${escapeHtml(x.campaignScheduleId)}</small>${scheduleStatusBadge_(x.scheduleStatus)}</div><div class="schedule-row-actions"><button type="button" class="secondary-action-button compact" data-schedule-clear="${escapeHtml(x.campaignScheduleId)}">Clear from History</button></div></div>`).join(''):'<div class="schedule-empty">No completed or canceled times in this view.</div>';
 const all=document.getElementById('campaignScheduleClearAll');if(all)all.hidden=!past.length;
}
async function clearCampaignScheduleHistory_(id,button){
 const row=campaignScheduleState.schedules.find(x=>x.campaignScheduleId===id),single=Boolean(id);
 const confirmed=await openDashboardConfirm({eyebrow:'Schedule history',title:single?'Clear this history item?':'Clear all schedule history?',message:single?`${formatCampaignScheduleDate_(row?.startDate)} at ${row?.startTime} will be hidden from this view. Its original record remains for audit.`:'All completed and canceled times for this campaign will be hidden from this view. Original records remain for audit. Upcoming schedules stay visible.',confirmLabel:single?'Clear Item':'Clear All History',destructive:true});if(!confirmed)return;
...
function renderReviewSchedules_(){
  ...
  const terminal=['CANCELED','COMPLETED','ARCHIVED'].includes(getRawCampaignStatus(getCampaignBuilderCampaign()));
  ...
  const timezone=(getCurrentCampaignSettings()||normalizeCampaignSetting({})).timezone;
  ...
  const done=document.getElementById('campaignReviewDone');if(done)done.textContent=rows.length?'Finish Scheduling':'Return to Campaigns';
...
async function saveReviewSchedule_(event){
  ...
  const tz=(getCurrentCampaignSettings()||normalizeCampaignSetting({})).timezone;
  ...
  await withActionButtonBusy(event.currentTarget,'Saving…',async()=>{
    try{
      ...
      campaignScheduleState.schedules=(result.schedules||[]).map(normalizeCampaignSchedule);campaignScheduleState.loaded=true;campaignScheduleState.error=null;
      const campaign=getCampaignBuilderCampaign();if(campaign&&['DRAFT','READY'].includes(getRawCampaignStatus(campaign)))campaign.campaignStatus='SCHEDULED';
...
function renderCampaignReview(result,queueStatus){
  ...
  const done=document.getElementById('campaignReviewDone');if(done){const upcoming=(campaignScheduleState.schedules||[]).some(x=>String(x.campaignId)===String(campaignBuilderCampaignId)&&x.scheduleStatus==='UPCOMING');done.textContent=upcoming?'Finish Scheduling':'Return to Campaigns';done.disabled=false;}
...
function attachCampaignReviewListeners(){
  ...
  document.getElementById('campaignReviewDone')?.addEventListener('click',async()=>{
    const scheduled=(campaignScheduleState.schedules||[]).some(x=>String(x.campaignId)===String(campaignBuilderCampaignId)&&x.scheduleStatus==='UPCOMING');
    const campaign=getCampaignBuilderCampaign();
    const name=campaign?.campaignName||'Campaign';
    await closeCampaignBuilder();
    switchView('campaignsView');
    activeCampaignLifecycleFilter=scheduled?'scheduled':'all';renderCampaignManagement();
    showCampaignsNotice(scheduled?`${name} scheduled successfully. You can change its send time from the campaign schedule.`:`${name} saved. You can return to edit or send it later.`,'success');
  });
